...
class NumberOfDelaysServiceRequestMetricMethodsImpl extends IntegerServiceRequestMetricMethodsImpl {
  ...
  override function updateMetricValue(helper : MetricUpdateHelper) : Date {
    if (Metric.ServiceRequest.Progress != TC_DRAFT and (Metric.New or helper.updateContainsChangesOfType(ServiceRequestChange))) {
      ...
      Metric.IntegerValue += countQuoteECDDelays(orderedHistory.where(\ c -> c.New_ExpectedQuoteCompletionDateGw != null))
      Metric.IntegerValue += countServiceECDDelays(orderedHistory.where(\ c -> c.New_ExpectedServiceCompletionDateGw != null))
    }
    return null
  }
  ...
  private function countQuoteECDDelays(history : List<ServiceRequestChange>) : int {
    ...
    updateECDChanges.each(\ ch -> {
      var priorDate = history.lastWhere(\ c -> c.Sequence < ch.Sequence).New_ExpectedQuoteCompletionDateGw
      if (priorDate != null and ch.New_ExpectedQuoteCompletionDateGw.after(priorDate)) {
        count++
      }
    })
    return count
  ...
  private function countServiceECDDelays(history : List<ServiceRequestChange>) : int {
    ...
    var count = 0
    updateECDChanges.each(\ ch -> {
      var priorDate = history.lastWhere(\ c -> c.Sequence < ch.Sequence).New_ExpectedServiceCompletionDateGw
      if (priorDate != null and ch.New_ExpectedServiceCompletionDateGw.after(priorDate)) {
        count++
      }
    })
    return count    
