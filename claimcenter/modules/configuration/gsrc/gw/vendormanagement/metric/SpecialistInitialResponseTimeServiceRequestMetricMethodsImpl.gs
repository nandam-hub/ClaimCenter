...
class SpecialistInitialResponseTimeServiceRequestMetricMethodsImpl extends TimeBasedServiceRequestMetricMethodsImpl {
  ...
  private property get HistoryChangesWithNewProgressOrExpectedCompletionDateValues() : List<ServiceRequestChange> {
    return Metric.ServiceRequest.History.where(\ change -> change.Progress_Chg or change.ExpectedQuoteCompletionDateGw_Chg or change.ExpectedServiceCompletionDateGw_Chg).orderBy(\ change -> change.Sequence)
  }
  
  private function delayWasAdded(change : ServiceRequestChange) : boolean {
    return (change.ExpectedQuoteCompletionDateGw_Chg or change.ExpectedServiceCompletionDateGw_Chg) and not change.Progress_Chg
  }
}