...
class QuoteTimelinessServiceRequestMetricMethodsImpl extends TimeBasedServiceRequestMetricMethodsImpl {
  ...
  override function updateMetricValue (helper : MetricUpdateHelper) : Date {
    ...
    if (Metric.New or helper.updateContainsChangesOfType(ServiceRequestChange)) {
      ...
      if (instructedChange != null) {
        ...
        updateLimitsIfECDChanged(ServiceRequest#ExpectedQuoteCompletionDateGw)
        var lastChange = changes.last()
        var terminalChange = ((lastChange.Operation == TC_ADDQUOTE) or (Metric.ServiceRequest.isProgressTerminal(lastChange.New_Progress) and Metric.IsOpen)) ? lastChange : null
        if (terminalChange != null) {
          ...
          if (isTerminatedButNotComplete(Metric.ServiceRequest, terminalChange.New_Progress)) {
  ...
  override property get BaseValueForLimitCalculation() : Integer {
    return (Metric.StartTime == null or Metric.ServiceRequest.ExpectedQuoteCompletionDateGw == null) ? null
                                                        : DateCalculator.timeBetween(Metric.StartTime, Metric.ServiceRequest.ExpectedQuoteCompletionDateGw, BusinessCalendarAddress)
  }
