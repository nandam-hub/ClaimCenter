...
class ServiceTimelinessServiceRequestMetricMethodsImpl extends TimeBasedServiceRequestMetricMethodsImpl {
  ...
  override function updateMetricValue (helper : MetricUpdateHelper) : Date {
    ...
    if (Metric.New or helper.updateContainsChangesOfType(ServiceRequestChange)) {
      ...
      if (startingChange != null) {
        ...
        updateLimitsIfECDChanged(ServiceRequest#ExpectedServiceCompletionDateGw)
        var terminalChange = (Metric.ServiceRequest.isProgressTerminal(changes.last().New_Progress)) ? changes.last() : null
  ...
  override property get BaseValueForLimitCalculation() : Integer {
    return (Metric.StartTime == null or Metric.ServiceRequest.ExpectedServiceCompletionDateGw == null)
        ? null
        : DateCalculator.timeBetween(Metric.StartTime, Metric.ServiceRequest.ExpectedServiceCompletionDateGw, BusinessCalendarAddress)
  }
