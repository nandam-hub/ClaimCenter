...
class PolicySearchConverter {
  ...
  public function createPCSearchCriteria( ccCriteria : PolicySearchCriteria ) : CCPCSearchCriteria {
    ...
    pcCriteria.AccountNumber = ccCriteria.AccountNumber
...
*/
  ...
  public function convertPCPolicy(pcPolicy : CCPolicy) : Policy {
    if(null == pcPolicy.PolicyType){
      pcPolicy.PolicyType = 'USParametirc'
    }
    if(pcPolicy.PolicyType == 'USAPersonalAuto'){
      pcPolicy.PolicyType = 'PersonalAuto'
    }
    ...
    if(policy.PolicyType == null){
      policy.PolicyType = PolicyType.TC_USPARAMETIRC
    }
    return policy
  }
