...
abstract class BaseLocalDocumentMetadataSource implements IDocumentMetadataSourceBase, InitializablePlugin {
  ...
  construct() {
    PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.info("BaseLocalDocumentMetadataSource started")
  }
  ...
  override property set Parameters( parameters : Map ) {
    if (parameters != null) {
      if (parameters.containsKey(DMS_DIR_PROP)) {
        ...
        if (!_metadata_dir.exists() && !_metadata_dir.mkdirs()) {
          PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.warn("Failed to create metadata directory '${_metadata_dir}'")
        }
      }
    ...
    PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.info("BaseLocalDocumentMetadataSource MetadataDir ${_metadata_dir}")
  }

  /** This will match the document to the criteria.
  *
  * IMPLEMENTATION NOTE:  This should be overridden in sub classes and call super.
  */
  ...
  protected property get MetadataFiles() : File[] {
    ...
    if (PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.DebugEnabled) {
      PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.debug("BaseLocalDocumentMetadataSource MetadataFiles ${rtn?.toList()}")
    }
    return rtn
  }
  ...
  override function retrieveDocument( uniqueId : String ) : Document  {
    PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.debug("BaseLocalDocumentMetadataSource retrieveDocument ${uniqueId}")
  ...
  override function saveDocument( document : Document ) : void  {
    ...
    PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.debug("BaseLocalDocumentMetadataSource saveDocument ${docId}")
    ...
    if (documentFiles != null) {
      try {
        for (documentFile in documentFiles) {
          ...
          PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.debug("BaseLocalDocumentMetadataSource getMatchingDocuments doc ${doc}")
          if (doc != null
  ...
  */  
  private function retrieveDocument( metadataFile : File, doThrow : boolean) : Document {
    ...
    try {
      ...
      PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.info("BaseLocalDocumentMetadataSource retrieveDocument id=${result.ID} docUID=${result.DocUID} publicId=${result.PublicID}")
      if (result != null) {
      ...
      if (doThrow) {
        ...
        PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.warn("Could not get document metadata from " + metadataFile, e)
        return null
      }
    }
    finally {
      if (xml != null) {
        try {
          ...
          // ignore
          PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.debug("BaseLocalDocumentMetadataSource.storeDocument: " + e.Message, e)
        }
      }
    }
  }
  ...
  */
  protected function storeDocument(document : Document) {
    var metadataFile = getMetadataFileForUniqueId(document.PublicID)
    PLLoggerCategory.DOCUMENT_ADAPTER_PLUGIN.info( "BaseLocalDocumentMetadataSource.store id=${document.ID} docUUD=${document.DocUID} file=${metadataFile}" )
