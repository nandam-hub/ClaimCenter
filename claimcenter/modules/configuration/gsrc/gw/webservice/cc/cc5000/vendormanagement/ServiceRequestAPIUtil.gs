...
internal class ServiceRequestAPIUtil {
  ...
  function searchForServiceRequests(specialistId: String, searchCriteria: ServiceRequestSearchCriteria): ServiceRequestSearchResults {
    ...
    if (searchCriteria.ServiceRequestReferenceNumber.HasContent) {
      q.compare(ServiceRequest#ServiceRequestReferenceNumberGw, Equals, searchCriteria.ServiceRequestReferenceNumber)
    }
    ...
    if (searchCriteria.OnlyInactive) {
      restrictToTerminalProgressValues(q)
    }
    ...
    if (searchCriteria.OnlyOverdue) {
      ...
      excludeTerminalProgressValues(q)
      q.or(\ restriction -> {
        restriction.compare(ServiceRequest#ExpectedQuoteCompletionDateGw, LessThan, Date.CurrentDate)
        restriction.compare(ServiceRequest#ExpectedServiceCompletionDateGw, LessThan, Date.CurrentDate)
      })
    }
    ...
    if (searchCriteria.OnlyUnpaidInvoices) {
      ...
      q.or(\ restriction -> {
        excludeTerminalProgressValues(restriction)
    ...
    for (sr in queryBeanResults) {
      ...
      if (searchCriteria.OnlyActive or searchCriteria.OnlyCompleteWaitingForInvoice) {
        ...
        if (sr.Terminated and not stateHandler.AllowsInvoices) {
          continue
        }
      }
  ...
  function updateServiceRequestReferenceNumber(specialistId: String, serviceRequestNumber: String, newReferenceNumber: String) {
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      serviceRequest.ServiceRequestReferenceNumberGw = newReferenceNumber
    })
  }
  ...
  function getMessagesForSpecialist(specialistId: String): MessagesForSpecialistResults {
    ...
    for (message in messages) {
      if (activeStatusOfServiceRequest[message.ServiceRequest] == null) {
        if (message.ServiceRequest.Terminated and
  ...
  function addOrReplaceQuote(specialistId: String,
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      var serviceRequest = getServiceRequestEntity(specialistId, serviceRequestNumber, bundle)
      newQuote = serviceRequest.CoreSR.createMinimalServiceRequestQuote()
  ...
  protected function addInvoiceToServiceRequest(serviceRequest: ServiceRequest,
    ...
    var newInvoice = serviceRequest.CoreSR.createMinimalServiceRequestInvoice()
    newInvoice.Status = ServiceRequestInvoiceStatus.TC_APPROVED
  ...
  private function addInvoiceToClaim(specialistId: String, claim: Claim, incident: Incident, exposure: Exposure, services: List<String>, creationInstructions: StatementCreationInstructions): String {
    ...
    var candidateSRs = exposure.ServiceRequestsGw ?: (incident.ServiceRequestsGw ?: claim.ServiceRequests)
  ...
  protected function selectServiceRequestFromCandidates(specialistId: String,
    ...
    candidateSRs = candidateSRs.where(\ srvReq -> {
      ...
      var addInvoiceOperationAvailable = srvReq.CoreSR.operationAvailable(ServiceRequestOperation.TC_ADDINVOICE, true)
  ...
  function updateExpectedCompletionDate(specialistId: String, serviceRequestNumber: String, newDate: Date, isService: boolean, changeReason: String) {
    Transaction.runWithNewBundle(\ bundle -> {
      var serviceRequest = getServiceRequestEntity(specialistId, serviceRequestNumber, bundle)
      var originalDate = isService ? serviceRequest.ExpectedServiceCompletionDateGw : serviceRequest.ExpectedQuoteCompletionDateGw
  ...
  function declineWork(specialistId: String,
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      var serviceRequest = getServiceRequestEntity(specialistId, serviceRequestNumber, bundle)
      var context = new ServiceRequestOperationContext() { :Reason = reason }
      serviceRequest.CoreSR.performOperation(TC_SPECIALISTDECLINED, context, true)
    })
  }
  ...
  function recordWorkResumed(specialistId: String,
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      var serviceRequest = getServiceRequestEntity(specialistId, serviceRequestNumber, bundle)
      serviceRequest.CoreSR.performOperation(TC_SPECIALISTRESUMEDWORK, null, true)
    })
  }
  ...
  function recordWaiting(specialistId: String,
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      var serviceRequest = getServiceRequestEntity(specialistId, serviceRequestNumber, bundle)
      var context = new ServiceRequestOperationContext() { :Reason = reason }
      serviceRequest.CoreSR.performOperation(TC_SPECIALISTWAITING, context, true)
    })
  }
  ...
  function recordWorkCompleted(specialistId: String, serviceRequestNumber: String) {
    Transaction.runWithNewBundle(\ bundle -> {
      var serviceRequest = getServiceRequestEntity(specialistId, serviceRequestNumber, bundle)
      serviceRequest.CoreSR.performOperation(TC_SPECIALISTCOMPLETEDWORK, null, true)
    })
  }
  ...
  function cancelServiceRequest(specialistId: String, serviceRequestNumber: String, reason: String) {
    Transaction.runWithNewBundle(\ bundle -> {
      var serviceRequest = getServiceRequestEntity(specialistId, serviceRequestNumber, bundle)
      var context = new ServiceRequestOperationContext() { :Reason = reason }
      serviceRequest.CoreSR.performOperation(TC_SPECIALISTCANCELED, context, true)
    })
  }
  ...
  function withdrawInvoice(specialistId: String,
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      var context = new ServiceRequestInvoiceOperationContext() { :Reason = reason }
      invoicesResult.single().CoreSR.performOperation(TC_WITHDRAWINVOICE, context, true)
    })
  }
  ...
  protected function addVisibilityRestriction(serviceRequestQueryBuilder : IQueryBuilder) {
    ...
    serviceRequestQueryBuilder.or(\restriction -> ServiceRequest.TerminalProgressValues.eachKeyAndValue(\srKind, terminalProgresses ->
        restriction.and(\restriction1 -> {
          restriction1.compare(ServiceRequest#Kind, Equals, srKind)
          restriction1
              .subselect("ID", CompareIn, ServiceRequestChange, "ServiceRequest")
              .compare(ServiceRequestChange#New_Progress, NotEquals, ServiceRequestProgress.TC_DRAFT)
              .compareNotIn(ServiceRequestChange#New_Progress, terminalProgresses.toTypedArray())
        })))
  }
  ...
  private function excludeTerminalProgressValues(q : IQueryBuilder) {
    addTerminalProgressValuesRestriction(q, true)
  }


  private function restrictToTerminalProgressValues(q : IQueryBuilder) {
    addTerminalProgressValuesRestriction(q, false)
  }


  private function addTerminalProgressValuesRestriction(q : IQueryBuilder, exclude : boolean) {
    q.or(\restriction -> ServiceRequest.TerminalProgressValues.eachKeyAndValue(\srKind, terminalProgresses ->
        restriction.and(\restriction1 -> {
          restriction1.compare(ServiceRequest#Kind, Equals, srKind)
          if (exclude) {
            restriction1.compareNotIn(ServiceRequest#Progress, terminalProgresses?.toTypedArray())
          } else {
            restriction1.compareIn(ServiceRequest#Progress, terminalProgresses?.toTypedArray())
          }
        })))
  }
