...
final class ServiceRequestDetails {
  ...
  */
  ...
  construct(serviceRequest : ServiceRequest) {
    ...
    InstructionText = serviceRequest.Instruction.InstructionTextGw
    ServiceAddress = serviceRequest.Instruction.ServiceAddressGw.DisplayName
    ...
    while (orderedHistoryIterator.hasPrevious()) {
      ...
      if (change.Progress_Chg) {
        if (serviceRequest.isProgressTerminal(change.New_Progress)) {
    ...
    CustomerContactDetail = new gw.webservice.cc.cc5000.vendormanagement.ContactDetail(serviceRequest.Instruction.CustomerContactGw)
    ...
    CanceledReason = serviceRequest.CanceledReasonGw
  }
