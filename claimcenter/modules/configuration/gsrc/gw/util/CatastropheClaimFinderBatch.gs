...
class CatastropheClaimFinderBatch extends BatchProcessBase {
  ...
  protected var _catActivityPattern: ActivityPattern = ActivityPattern.finder.getActivityPatternByCode("catastrophe_review")
  protected static final var MAX_CLAIM_RESULTS: Integer = CCConfigParameters.MaxCatastropheClaimFinderSearchResults.Value
  protected static final var SYSTEM_USER_NAME: String = "sys"
  ...
  protected function findMarkedCatastrophes() : IQueryBeanResult<Catastrophe> {
    return Query.make(entity.Catastrophe).compare("ScheduleBatch", Equals, Boolean.TRUE).select()
  }

  protected function createReviewForCatastropheActivity(claim : Claim) {
  ...
  protected function getHistoryDescription(numClaimsProcessed : int, numClaimsMatched : int) : String {
  ...
  protected function createAndAddCatastropheClaimsHistory(catastrophe : Catastrophe, historyDescription : String) {
    gw.transaction.Transaction.runWithNewBundle( \ bundle -> {
  ...
  protected function clearScheduleBatch(catastrophe : Catastrophe) {
    gw.transaction.Transaction.runWithNewBundle( \ bundle -> {
      var cat = bundle.add(catastrophe)
