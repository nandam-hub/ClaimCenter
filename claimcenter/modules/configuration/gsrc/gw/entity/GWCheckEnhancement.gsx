...
enhancement GWCheckEnhancement : Check {
  ...
  private function setPrimaryPayeeRelatedFields() {
    ...
    if (this.PaymentMethod == PaymentMethod.TC_INSTANT and payee.Payee != null) {
      updateInstantPmtFields()
    }
  }


  /**
   * This method sets the InstantPmtPayeeCellPhone and InstantPmtPayeeEmail fields
   * based on the FirstPayee values. Also it will reset these fields and payment method
   * if the FirstPayee type is not Person
   */
  private function updateInstantPmtFields() {
    if (!(this.FirstPayee.Payee typeis Person)) {
      this.PaymentMethod = PaymentMethod.TC_CHECK
      this.InstantPmtPayeeCellPhone = ""
      this.InstantPmtPayeeEmail = ""
    } else if (CCConfigParameters.InstantPaymentIntegrationEnabled.Value){
      var payee = this.FirstPayee
      this.InstantPmtPayeeCellPhone = (payee.Payee as Person).CellPhone
      this.InstantPmtPayeeEmail = payee.Payee.EmailAddress1
    }
  }
  ...
  public function resetCloneFields( originalCheck : Check ) {
    ...
    this.InstantPmtExternalID = null
    this.InstantPmtVendorStatus = null
    this.InstantPmtVendorStatusDesc = null
  }
  ...
  */
  ...
  private function unlinkServiceRequestInvoice(changeDescription: String) {
    for (invoice in this.ServiceRequestInvoices) {
      invoice.CoreSR.performOperation(
          ServiceRequestOperation.TC_UNPAYINVOICE,
          new ServiceRequestInvoiceOperationContext() {:Reason = changeDescription},
          false,
          null,
          true
      )
