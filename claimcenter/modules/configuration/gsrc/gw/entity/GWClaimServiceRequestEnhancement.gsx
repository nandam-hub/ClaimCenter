...
enhancement GWClaimServiceRequestEnhancement : entity.Claim {
  ...
  function newServiceRequest(custContact : Contact, incident : Incident, exposure : Exposure = null) : ServiceRequest {
    ...
    var instruction = new ServiceRequestInstruction() {
      :CustomerContactGw = contact
    }
    var serviceRequest = this.CoreSR.createMinimalServiceRequest();
    serviceRequest.IncidentGw = incident
    serviceRequest.ExposureGw = exposure
    serviceRequest.Instruction = instruction
    serviceRequest.addToInstructionHistory(instruction)
    serviceRequest.RequestedQuoteCompletionDateGw = serviceRequest.DefaultRequestedQuoteCompletionDate
    serviceRequest.RequestedServiceCompletionDateGw = serviceRequest.DefaultRequestedServiceCompletionDate
    ...
    if (not this.DraftClaim) {
      this.ServiceRequests.each(\ sr -> { 
        if (sr.New and sr.Progress != ServiceRequestProgress.TC_REQUESTED) {
          sr.CoreSR.performOperation(TC_SUBMITINSTRUCTION, null, false)
        }})
    }
  }
