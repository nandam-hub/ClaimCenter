...
class CatastropheMethodsImpl implements CatastropheMethods {
  ...
  override function updatePolicyLocationSummaryJoin() {
    if (_claim.State == TC_DRAFT or
        (CatastropheProperties.INSTANCE.FeatureEnabled and _claim.CatFromCatClaimTemplate_SP and _claim.New)) {
      // avoid CDCE in FNOL Wizard
      // or when a new claim is created from the Proactive Cat Claim Accelerator feature.
      // (In the accelerator, we explicitly call the updatePolicyLocationSummaryJoin method below synchronously)
      return
    }
