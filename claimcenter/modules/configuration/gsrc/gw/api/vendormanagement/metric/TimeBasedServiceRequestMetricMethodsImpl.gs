...
class TimeBasedServiceRequestMetricMethodsImpl extends ServiceRequestMetricMethodsImpl implements TimeBasedMetricMethods {
  ...
  /**
   * Return whether the provided ServiceRequest would be considered terminated but not complete if it were at the supplied progress level.
   * @param serviceRequest - a ServiceRequest
   * @param progress - The progress level to test
   * @return true if it would be, false otherwise
   */
  protected function isTerminatedButNotComplete(serviceRequest : ServiceRequest, progress : ServiceRequestProgress) : boolean {
    return progress != TC_WORKCOMPLETE and serviceRequest.isProgressTerminal(progress)
  }
  ...
  private function findTerminatedDate() : Date {
    if (Metric.ServiceRequest.Terminated) {
